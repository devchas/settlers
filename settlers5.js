...
$('html').click(function(e) {                    
	if(!$(e.target).hasClass('setRdColor') ) {
    	roadColor = null;
	} 
	if (!$(e.target).hasClass('settleColor') ) {
		stmntColor = null;
	}
	if (!$(e.target).hasClass('cityColor') ) {
		cityColor = null;
	}
}); 
		...
		if (this.getContext) {
			...
			this.addEventListener('click', function(event) {
				if (roadColor != null) { 
					...
					roadCanv.strokeStyle = roadColor;
					roadCanv.stroke();
					roadColor = null;
					this.isSet = true;
				} else {
...
function initSettlements() {
	...
	$('.settle').each(function() {
		...
		if (this.getContext) {
			...
			this.addEventListener('click', function(event) {
				if (stmntColor != null) { 
					settCanv.fillStyle = stmntColor;
					createSett(settCanv, sWidth, sHeight);
					stmntColor = null;
					this.isSet = true;
				} else if (cityColor != null) {
					settCanv.fillStyle = cityColor;
					createCity(settCanv, sWidth, sHeight);
					cityColor = null;
					this.isSet = true;
				} else {
					settCanv.canvas.width = sWidth;
...
function createSett(c, width, height) {
	height = height - 1;
	var faceWdth = width * .8;
	var faceHt = height * .65;
	c.beginPath();
	c.moveTo((width - faceWdth) / 2, height);
	c.lineTo(width - (width - faceWdth) / 2, height);
	c.lineTo(width - (width - faceWdth) / 2, height - faceHt);
	c.lineTo(width / 2, 0);
	c.lineTo((width - faceWdth) / 2, height - faceHt);
	c.closePath();
	c.stroke();
	c.fill();
}

function createCity(c, width, height) {
	width = width - 1;
	height = height - 1;
	var ledgeWdth = width * .5;
	var tpWdth = width * .75;
	var ledgeHt = height * .45;
	var stkHt = height * .3;
	c.moveTo(1, height);
	c.lineTo(width, height);
	c.lineTo(width, stkHt);
	c.lineTo(tpWdth, 0);
	c.lineTo(ledgeWdth, stkHt);
	c.lineTo(ledgeWdth, ledgeHt);
	c.lineTo(1, ledgeHt);
	c.closePath();
	c.stroke();
	c.fill();
